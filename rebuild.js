...
fs.readFile('lib/sisal.pegjs', 'utf8', function(err, data) {
  ...
  fs.writeFile('lib/lexer.js', 'lexer = ' + parser + ';', function(err) {
    if (err)
      return console.log(err);
...
fs.readFile('node_modules/indent-adder/index.js', 'utf8', function(err, data) {
  if (err)
    return console.log(err);
  if (data.indexOf("exports.add_indents =") === -1)
    throw "Something is wrong with indent-adder";
  fs.writeFile('lib/indenter.js',
