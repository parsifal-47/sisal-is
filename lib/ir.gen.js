helper = {
  ...
  add : function(source, addition) {
    ...
    if (helper.isArray(addition))
      merged = merged.concat(addition);
    else if (!addition.fake)
  ...
  merge : function(source, left, right) { // three arrays
    ...
    if (helper.isArray(left))
      merged = merged.concat(left);
    else if (!left.fake)
      merged.push(left);

    if (helper.isArray(right))
      merged = merged.concat(right);
    else if ((right) && (!right.fake))
      merged.push(right);
    ...
    if (obj instanceof Object) {
      ...
      for (var attr in obj) {
        if (obj.hasOwnProperty(attr))
          copy[attr] = helper.clone(obj[attr]);
      }
      return copy;
    }
...
irGenerator = function() {
  ...
  this.parseType =
    ...
    if (astNode.type === "ArrayOf") {
      ...
      this.parseRangeGen =
          function(astNode, inputs) { // Range gen is a special case because it
                                      // contains scatter and dot nodes
    var outPorts = [];
    var subnodes = [];
    var gen = null;
    ...
    if (astNode.type === "RangeFromSource") {
      ...
      for (var i = 0; i < astNode.names.length; i++)
        outPorts.push(
            new port.typedName(astNode.names[i], type.simple("auto")));
    ...
    return gen;
          ...
          this.parseComplex =
              function(astNode, complex, inputs) {
    if (!astNode)
      return new node.fake(0);
    ...
    if (helper.isArray(astNode)) {
      ...
      throw "Unexpected kind of complex node"
              ...
              this.parseLetInit =
                  function(deflist, inputs) {
    var let = new node.virtualComplex();
    var newInputs = helper.clone(inputs);
    ...
    if (deflist !== null) {
      ...
      for (var i = 0; i < deflist.length; i++) {
        ...
        var tp = new port.typedName(
            (newInput[0] ? newInput[0] : newInput).outPorts[0].name,
            type.simple("auto"));
    ...
    return {node : let , newInputs : newInputs};
                  ...
                  this.parseBody =
                      function(astNode,
                               inputs) { // Body is special case, it conatins
                                         // self-created ports as "old" values
    return self.parseComplex(astNode, new node.loopBody(inputs, []), inputs);
  }

                      this.parseReturns =
                          function(astNode, inputs) {
    return self.parseComplex(
        astNode.expressions,
        new node.loopReturn(astNode.reduction.name, inputs, []), inputs);
  }

                          this.parseLoopCondition =
                              function(name, astNode, inputs) {
    return self.parseComplex([ astNode ],
                             new node.typedComplex(name, inputs, []), inputs);
  }
    ...
    return self.parseComplex(
        ...
        new node.loopReturn(astNode.reduction.name, inputs, []), inputs);
                                  ...
                                  this.parse =
                                      function(astNode, inputs, undefined) {
    if (helper.isArray(astNode)) { // Parse as array of parsed instances
      var arr = [];
      for (var i = 0; i < astNode.length; i++) {
    ...
    if (helper.isString(astNode) && astNode) {
      if (helper.isArray(inputs)) {
        for (var i = 0; i < inputs.length; i++) {
          if (inputs[i].name == astNode)
            return new node.fake(inputs[i].color);
        }
      }
    ...
    if (astNode === null)
      return new node.virtualComplex();
    ...
    case "If":
      ...
      var ifnode = new node.if(condition, thenbranch, elsebranch, elseif,
                               inputs, [ new port.colored(color.getNew()) ]);
      return ifnode;

    case "ForAll":
      var range = self.parseRangeGen(astNode.range, inputs);
      var body =
          self.parseBody(astNode.body, helper.merge(range.outPorts, inputs));
      var returns = self.parseReturns(
          astNode.returns,
          helper.merge((body ? body.outPorts : []), range.outPorts, inputs));
      return new node.forAll(range, body, returns, inputs,
                             [ new port.colored(color.getColor(returns)) ]);
    ...
    case "For":
      ...
      var condition = self.parseLoopCondition("condition", astNode.condition,
                                              extendedInputs);
      var postcondition = self.parseLoopCondition(
          "postcondition", astNode.postcondition, extendedInputs);
      var body = self.parseBody(astNode.body, extendedInputs);
      var returns = self.parseReturns(
          astNode.returns, helper.merge((body ? body.outPorts : []),
                                        init.outPorts, extendedInputs));
      return new node.for(init, condition, postcondition, body, returns, inputs,
                          [ new port.colored(color.getColor(returns)) ]);
    ...
    case "BinaryExpression":
      ...
      var op = new node.binary(astNode.operator,
                               [
                                 new port.colored(color.getColor(left)),
                                 new port.colored(color.getColor(right))
                               ],
    ...
    case "[]":
      ...
      if (astNode.type === "Call")
        tuple = [ new node.call(astNode.name, [],
                                new port.colored(color.getNew())) ];
    ...
    case "Dot":
      ...
      var dot = new node.unary(".", new port.colored(dotcolor),
                               new port.colored(color.getNew()));
      return [ dot, field ];

    case "Unary":
      var operand = self.parse(astNode.operand);
      var op = new node.unary(astNode.operation,
                              new port.colored(color.getColor(operand)),
                              new port.colored(color.getNew()));
      return helper.add([ op ], operand);

    case "Definition":
      var def = self.parse(astNode.right, inputs);
      (def[0] ? def[0] : def).outPorts[0].name =
          astNode.left[0]; // out port comes first that's why zero
      // TODO: fix 0
      return def;
    ...
    default:
      throw "Unexpected node type: " + astNode.type;
    }
  }

                                      this.notConnectedOutput =
                                          function(cNode, nodeNum, outPortNum) {
    for (var i = 0; i < cNode.edges.length; i++) {
      if (cNode.edges[i].nodeFrom == nodeNum &&
          cNode.edges[i].portFrom == outPortNum)
        return false;
    }
    return true;
  }

                                          this.createAndConnectOutput =
                                              function(cNode) {
    var outPortNum = 0;
    ...
    if (helper.isArray(cNode.nodes)) {
      for (var l = 0; l < cNode.nodes.length; l++) {
        if (!helper.isArray(cNode.nodes[l].outPorts))
          continue;

        for (var i = 0; i < cNode.nodes[l].outPorts.length; i++) {
          if (!cNode.nodes[l].outPorts[i].color)
            continue;
          for (var j = 0; j < cNode.nodes.length; j++) {
            for (var k = 0; k < cNode.nodes[j].inPorts.length; k++) {
              if (cNode.nodes[l].outPorts[i].color ==
                  cNode.nodes[j].inPorts[k].color) {
                cNode.edges.push(new edge.single(l, i, j, k));
              }
            }
            if (!cNode.nodes[l].fake && cNode.nodes[j].fake &&
                cNode.nodes[j].color &&
