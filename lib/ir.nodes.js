node = {
  complex : {
    // Interface class for all complex nodes
    addNodes : function(node) {
      ...
      if (node.pureVirtual) { // node instanceof virtualComplexNode) { -- this
                              // typing is not working, using duck typing
        var increment = this.nodes.length;
        for (var i = 0; i < node.nodes.length; i++)
          this.nodes.push(node.nodes[i]);
        // copy variables
        for (var i = 0; i < node.edges.length; i++) {
          this.edges.push(new edge.single(
              node.edges[i].nodeFrom === -1
                  ? -1
                  : node.edges[i].nodeFrom + increment,
              portFrom,
              node.edges[i].nodeTo === -1 ? -1
                                          : node.edges[i].nodeTo + increment,
              portTo));
        }
      } else
    ...
    removeNode : function(nodeId) {
      ...
      while (i < this.edges.length) {
        if ((this.edges[i].nodeFrom == nodeId) ||
            (this.edges[i].nodeTo == nodeId)) {
          this.edges.splice(i, 1);
        } else {
          if (this.edges[i].nodeFrom > nodeId)
            this.edges[i].nodeFrom--;
          if (this.edges[i].nodeTo > nodeId)
            this.edges[i].nodeTo--;
          i++;
        }
      }
    ...
    overConnect : function(nodeId) {
      if (this.nodes[nodeId].inPorts.length == 0 ||
          this.nodes[nodeId].outPorts.length == 0)
        return;
      for (var i = 0; i < this.edges.length; i++)
        for (var j = 0; j < this.edges.length; j++) {
          if (this.edges[i].nodeTo == nodeId &&
              this.edges[j].nodeFrom == nodeId &&
              this.edges[i].portTo == this.edges[j].portFrom) {
            this.edges.push(
                new edge.single(this.edges[i].nodeFrom, this.edges[i].portFrom,
    ...
    toXML : function(id) {
      ...
      if (typeof this.fields !== "undefined")
        for (i = 0; i < this.fields.length; i++)
          data[this.fields[i]] = this[this.fields[i]];

      for (i = 0; i < this.inPorts.length; i++)
        innerXML += graphmlgen.port('in' + i);
      for (i = 0; i < this.outPorts.length; i++)
        innerXML += graphmlgen.port('out' + i);
      for (i = 0; i < this.nodes.length; i++)
        innerXML += this.nodes[i].toXML(id + '::n' + i);
      for (i = 0; i < this.edges.length; i++)
        innerXML += edge.toXML(id, this.edges[i]);
    ...
    execute : function() {
      ...
      do {
        changed = 0;
        for (i = 0; i < this.nodes.length; i++)
          if (((typeof this.nodes[i].ready == "undefined") ||
               this.nodes[i].ready()) &&
              left.indexOf(i) !== -1) {
            ...
            for (j = 0; j < this.edges.length; j++)
              if ((this.edges[j].nodeFrom === i) &&
                ...
                if (nodeTo === -1)
                  this.outPorts[this.edges[j].portTo] =
                      this.nodes[i].outPorts[this.edges[j].portFrom].value;
                else
    ...
    toText : function() {
      var inFree = 0,
          outFree = 0; // Number of non-assigned input and output ports
      for (i = 0; i < this.outPorts.length; i++)
        if (!this.outPorts.value)
          outFree++;
      for (i = 0; i < this.inPorts.length; i++)
        if (!this.inPorts.value)
          inFree++;
      if (inFree === 0) {
...
node.func =
    function(name, inPorts, outPorts) {
  var self = this;
  this.type = "function";
  this.name = name;
...
node.forAll =
    function(range, body, returns, inPorts, outPorts) {
  this.type = "forall";
  this.inPorts = inPorts;
  this.outPorts = outPorts;
...
node.for =
    function(init, condition, postcondition, body, returns, inPorts, outPorts) {
  this.type = "forall";
  this.inPorts = inPorts;
  this.outPorts = outPorts;
...
node.if =
    function(condition, ifbranch, elsebranch, elseif, inPorts, outPorts) {
  this.type = "if";
  this.inPorts = inPorts;
  this.outPorts = outPorts;
...
node.binary =
  ...
  this.fields = [ "op" ];
  this.toText = function() {
    if (typeof this.outPorts[0].value != "undefined")
      return this.outPorts[0].value;
    else
      return this.op;
  } this.ready = function() {
    return ((typeof this.inPorts[0].value != "undefined") &&
            (typeof this.inPorts[1].value != "undefined"));
...
node.identifier =
    function(id, color) { // port with name
  this.type = "id";
  this.inPorts = [];
...
node.constant =
    function(value, color) { // port with name
  var self = this;
  this.type = "constant";
  this.inPorts = [];
  this.outPorts = [ new port.colored(color) ];
  ...
  this.color = color;
  this.fields = [ "value" ];
  this.toText = function() { return this.value; } this.execute =
      function() { this.outPorts[0].value = this.value; }
}
...
node.sisalir = function(ast) { // constructs sisal IR from Abstract syntax tree
  ...
  } this.execute = function() { return self.container.execute(); } this.toText =
          function() { return self.container.toText(); }
}
